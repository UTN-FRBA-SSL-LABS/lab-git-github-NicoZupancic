...
> R:Porque git revert no reescribe la historia, mientras que git reset --hard sí lo hace. 
...
> R:Ambas funciones verifican si un número es par, pero una usa el módulo (n % 2 == 0) y la otra operaciones de bits (n & 1 == 0). La primera es más legible, la segunda es más “baja nivel” y puede ser más eficiente.
...
> R:Le diría que es mejor hacer commits chicos y con un solo propósito, porque así el código es más fácil de revisar, depurar y mantener.

**P4** — ¿Cuál es la diferencia entre `git fetch` y `git pull`? ¿Cuándo preferirías usar uno sobre el otro?

> R:git fetch descarga los cambios del repositorio remoto pero no modifica tu código local, solo actualiza las referencias. En cambio, git pull hace fetch + merge, es decir, descarga los cambios y los integra automáticamente a tu rama actual. Usaría git fetch cuando quiero ver qué cambió en el remoto antes de mezclarlo con mi trabajo (más control y seguridad). Usaría git pull cuando estoy seguro de que puedo integrar esos cambios directamente sin revisar demasiado primero.

**P5** — ¿Qué información debería tener la descripción de un Pull Request para que sea útil para quien lo revisa?

> R:La descripción de un Pull Request debería explicar qué cambio se hizo y por qué, no solo mostrar el código. Tiene que incluir qué problema resuelve o qué funcionalidad agrega, cómo se probó el cambio y cualquier decisión importante de implementación que ayude al reviewer a entender el contexto
